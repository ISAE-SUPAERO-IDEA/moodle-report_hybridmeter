...
Vue.component('category', {
  ...
  async created() {
    this.tree = await this.get(`/report/hybridmeter/tree_handler.php?task=category_children&id=${this.id}`);
  ...
  methods: {
    ...
    async manage_category_blacklist(category) {
      if (!this.global_blacklist) {
        ...
        category.blacklisted = (await this.get(`/report/hybridmeter/tree_handler.php?task=manage_blacklist&type=category&id=${category.id}&value=${value}`)).blacklisted;
    ...
    async manage_course_blacklist(course) {
      if (!this.global_blacklist) {
        ...
        course.blacklisted = (await this.get(`/report/hybridmeter/tree_handler.php?task=manage_blacklist&type=course&id=${course.id}&value=${value}`)).blacklisted;
        this.tree = Object.assign({}, this.tree);
      }
    },
